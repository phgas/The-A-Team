...
if __name__ == "__main__":
    is_valid_prediction_data, prediction_data_raw = read_prediction_data("prediction_data.csv")
    if is_valid_prediction_data:
        ...
        prediction = get_prediction(model, prediction_data_prepared)
        print(prediction)
        display(prediction)

    """
    is_valid_training_data, training_data_raw = read_training_data("ai4i2020.csv")
    if is_valid_training_data:
        training_data_prepared = prepare_data(training_data_raw)
        model = create_new_model(training_data_prepared, "ai4i2020_pycaret_model")
    """